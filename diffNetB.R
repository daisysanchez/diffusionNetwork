...
# must  be formatted as 1st column: year, following columns of authors in that reference
  ...
  for(i in 1:nrow(CSV)){
    ...
    for(j in 2:ncol(CSV)){ 
      ...
      if(CSV[i,j] != ' ' && CSV[i,j] != '' &&  !is.null(CSV[i,j]) && !is.na(CSV[i,j])){
        ...
        auth = paste('^', auth, '$', sep = '') #raps auth name in regex so that it can be compared
          ...
          if(length(grep(auth, authorYear$author, ignore.case = TRUE)) != 0){ #if auth exists in authyear look for id
            temp = grep(auth, authorYear$author, ignore.case = TRUE) #returns the indices at which the name appear
            min = which.min(temp) #grabs the first index
            minIndex = temp[min] #grabs the row number
            id = authorYear[minIndex, 3] #sets the id to exisiting id so that it can be assigned correctly
    ...
    } #end of coloumn file traversal
  } #end of csv file traversal
  ...
  #deletes variables that will no longer be used again
  rm(auth, count, i, id, j, min, minIndex, temp, year, newRow)
...
#CREATES EDGELIST FROM AUTHORYEAR DATAFRAME
  ...
  for(i in 1:nrow(authorYear)){
    ...
    j = i+1 #sets j to row after current
  ...
  #deletes variables that will no longer be used again
  rm(authAlter, authEgo, i, j, year, newRow)
  
  
  
#CREATES DIFFUSION NETWORK
  temp <- edgelist_to_adjmat( #creates adjacency matrix
    edgelist = authEdgelist[,1:2], #defines the edgelist as all the rows of authEdgelist, and its first two columns
    undirected = FALSE, #defines edgelist as directed
    t = nrow(authEdgelist) #defines the number of time periods as the number of rows
  )
  
  adjmat <- edgelist_to_adjmat( #creates adjacency matrix
    edgelist = authEdgelist[,1:2], #defines the edgelist as all the rows of authEdgelist, and its first two columns
    undirected = FALSE, #defines edgelist as directed
    t=nrow(temp[[1]]) #defines the number of time periods as the number of rows 
  )
  
  rm(temp)
  
  toa <- data.frame(matrix(ncol = 1, nrow = 0)) #creates toa list
  colnames(toa) <- c("toa") #renames column
  
  for(i in 1:nrow(adjmat[[1]])){ #populates the toa list with num rows of adjmat[[1]]
    toa[i,1] = i
  }
